...
const SuperAdminPage = () => {
  ...
  const handleExportStudents = async (batchName, format) => {
    if (format === 'excel') {
      try {
        const response = await api.get(`/batches/${encodeURIComponent(batchName)}/export`, {
          responseType: 'blob'
        });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', `${batchName}_students.xlsx`);
        document.body.appendChild(link);
        link.click();
        link.remove();
        window.URL.revokeObjectURL(url);
        toast.success(`Excel downloaded for ${batchName}`);
      } catch (error) {
      ...
      doc.text('ProgressPoint', 14, 13);

      // Right-aligned tagline
      doc.setFontSize(8);
      doc.setFont('helvetica', 'italic');
      doc.text('Student Batch Performance Report', pageW - 14, 9, { align: 'right' });
      doc.text('Confidential — Internal Use Only', pageW - 14, 15, { align: 'right' });
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.text(`Batch: ${batchName}`, 14, 29);
      doc.setFont('helvetica', 'normal');
      doc.text(`Total Students: ${studentsInBatch.length}`, pageW / 2, 29, { align: 'center' });
      doc.text(`Generated: ${new Date().toLocaleString('en-IN', { dateStyle: 'long', timeStyle: 'short' })}`, pageW - 14, 29, { align: 'right' });

      // ── Data table ─────────────────────────────────────────────────
      const rows = studentsInBatch.map((s, idx) => [
        idx + 1,
        s.regNo,
        s.name,
        s.department || '-',
      ...
      autoTable(doc, {
        startY: 36,
        head: [['S.No', 'Reg No', 'Name', 'Dept', 'Email', 'Efforts', 'Presentation', 'Assessment', 'Assignment', 'Attendance']],
        body: rows,
        ...
        didDrawPage: (data) => {
          ...
          doc.setFont('helvetica', 'italic');
          doc.text(
            'Generated by ProgressPoint  |  This report is confidential and intended for internal use only.',
      ...
      toast.success(`PDF downloaded for ${batchName}`);
    }
  };
            ...
            <div className="bg-base-100 rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-5xl max-h-[95vh] sm:max-h-[90vh] relative flex flex-col">
              ...
              <div className="p-3 sm:p-4 lg:p-6 border-b border-base-300">
                ...
                <div className="mt-3 sm:mt-4 flex flex-col sm:flex-row gap-2 justify-end">
                  <button
                    className="btn btn-success btn-outline btn-xs sm:btn-sm font-semibold flex items-center gap-1"
                    onClick={() => handleExportStudents(selectedBatch, 'excel')}
                  ...
                  <button
                    className="btn btn-error btn-outline btn-xs sm:btn-sm font-semibold flex items-center gap-1"
