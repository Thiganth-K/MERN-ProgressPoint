...
// Marks routes
router.post("/:batchName/student/:regNo/marks", requireAdminOrSuperAdmin, checkMarksTime, logAdminAction("update_marks", (req) => ({
  batchName: req.params.batchName,
  studentRegNo: req.params.regNo,
  action: "Update student marks",
  metadata: { marks: req.body.marks }
})), updateStudentMarks);
router.get("/:batchName/marks/:date", requireAdminOrSuperAdmin, logAdminAction("view_marks", (req) => ({
  batchName: req.params.batchName,
  action: `View marks for date ${req.params.date}`,
  metadata: { date: req.params.date }
})), getMarksForDate);
router.post("/:batchName/marks", requireAdminOrSuperAdmin, checkMarksTime, logAdminAction("update_marks", (req) => ({
  batchName: req.params.batchName,
